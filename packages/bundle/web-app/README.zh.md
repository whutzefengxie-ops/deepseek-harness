...
- **本地 `/review` 进程所有权只在 Windows 可用**：随附 Web 组合只在 Windows 挂载[审查者命令](../../interaction/command-reviewer/README.zh.md)，此时本地子进程提供方可提供 kill-on-close Job Object 所有权。POSIX 默认不挂载命令及其设置；拥有等价宿主死亡保证的部署可在后置 overlay 中启用该行。
