...
- **The local `/review` process owner is Windows-only** — the shipped Web composition mounts the [reviewer command](../../interaction/command-reviewer/README.md) only on Windows, where the local subprocess provider supplies kill-on-close Job Object ownership. POSIX omits both the command and its settings; a deployment with an equivalent Host-death provider can enable the row in a later overlay.
