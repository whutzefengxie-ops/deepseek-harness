...
- **The local `/review` process owner is Windows-only** — the shipped Web composition mounts the [reviewer command](../../interaction/command-reviewer/README.md), but its local subprocess provider rejects the run before Codex starts on POSIX and renders the durable failure in Chat. A POSIX deployment needs a subprocess provider with an equivalent Host-death guarantee.
