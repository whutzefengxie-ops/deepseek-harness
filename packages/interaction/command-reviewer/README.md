...
| `/review` with the shipped local provider on POSIX | The composer clears after admission, then the durable card fails with `The review could not start: subprocess-local: host-death termination requires Windows Job Object ownership`; Codex is not started. |
