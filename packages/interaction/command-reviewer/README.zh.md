...
每次成功准入的调用都会依次记录 `review/start`、零到多个从 Codex JSONL 解码出的 `review/activity`，以及一个 `review/end`；同时仍有执行器持有的纯日志事件对 `command/run` / `command/done`。`review/start.request` 保存本次 Codex 请求实际使用的完整提示词、规范 argv、显式子进程环境项、工作目录与运行超时。对话记录会排除指令、目录和运行时快照上下文，避免把这些配置误记为用户对话；直接消息、模型输出、工具活动、通知、转发、召回和压缩摘要仍作为审查证据。这些事件都不会进入被审查 Agent 的模型历史。处理器会在写入 `review/start` 前立即提交请求准入，因此浏览器断开不会留下“审查仍在运行、`command/done` 却为错误”的矛盾记录；该记录写入后，断开连接或刷新页面不会取消审查。接收 Agent 的上下文拥有后台进程；超时、Agent 卸载或插件卸载都会终止进程树，并等待整棵进程树退出后再记录终态。如果无法确认整棵进程树已经退出，本次运行不会发布误导性的终态，会继续占用并发名额，并在卸载期间报告清理失败。后续宿主恢复一个含有未闭合 `review/start` 的会话时，会先补充一条指向该启动事件的成功 `command/done`（若缺失），再追加一条 `interrupted` 终态；此前的进程已经不可能仍在运行，因此两个持久生命周期会同时闭合。
...
| `timeoutMs` | `1800000` | 单次审查的最长运行时间（毫秒）；超时后终止进程树，且受 Node 定时器上限约束。 |
| `maxConcurrentReviews` | `1` | 同一 Agent 可同时持有的活动审查数量上限。 |

提示词从不放进 argv。它以批量 stdin 的形式穿过子进程 seam，因此任何对话文本都不会进入 shell 边界。每次运行都会用不可预知的分隔标识包裹对话记录，并明确说明它是不可信证据、不得提供指令；这会降低提示词注入风险，但无法让可写沙箱模式安全地处理不可信对话。Codex 使用当前子进程提供方返回的规范路径启动。如果该路径是 Windows 批处理包装器，提供方还会解析 `cmd.exe`；argv 使用 `/d /q /v:off /s /c` 并展开 `DSH_CODEX_REVIEWER_EXECUTABLE`，显式子进程环境则携带加引号的规范包装器路径。该形式可以保留包含空格和命令元字符的路径，同时不会把对话文本放进 shell 命令。模型标识符会在进入解释器前完成校验。stdout 使用流式 JSONL 管道，stderr 则保留有界诊断尾部。Codex 运行是非交互、临时（ephemeral）、去除颜色、允许在 Git 仓库之外执行的；它在会话的工作目录（会话没有工作目录时用进程目录）中运行。
