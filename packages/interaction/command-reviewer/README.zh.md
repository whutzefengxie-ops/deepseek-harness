...
每条记录的活动都属于真实 Codex 条目，并将其外部 id 存为 `item:<id>`。该 id 的重复事件必须保持同一个活动类别。顶层 turn 生命周期记录会被忽略，因为它们并不标识独立的公开操作。一次运行最多记录 `maxActivityEvents` 个活动转换；首个超限转换会终止 Codex，并让卡片以超限诊断结束。
...
| Codex 运行失败 | 持久化审查卡片会以失败状态结束，并显示所有已观察到的 Codex 错误、超时、退出码或信号、畸形输出诊断、字节超限诊断和活动事件超限诊断；后续失败前已解码出的最终审查文本会保留在这些诊断上方。 |

每次成功准入的调用都会依次记录 `review/start`、零到多个从 Codex JSONL 解码出的 `review/activity`，以及一个 `review/end`；同时仍有执行器持有的纯日志事件对 `command/run` / `command/done`。每条终态记录都包含至少一个非空白字符的结果或诊断。该命令关闭通用输入记录，因此 `command/run` 省略 `args`；`review/start.focus` 是成功准入后关注点的唯一持久副本。`review/start.request` 保存本次 Codex 请求实际使用的完整提示词、规范 argv、显式子进程环境项、工作目录、必需的 `hostDeath: "terminate"` 行为与运行超时。对话记录会排除指令、目录和运行时快照上下文，避免把这些配置误记为用户对话；直接消息、模型输出、工具活动、通知、转发、召回和压缩摘要仍作为审查证据。这些事件都不会进入被审查 Agent 的模型历史。处理器会在写入 `review/start` 前立即提交请求准入，要求会话存储执行 flush，再携带审查所有者的取消信号从 `ctx.sessionPersistence` 的新事件序号开始读取；只有存储事件与该 `review/start` 完全匹配且所有 flush listener 都成功时才会启动 Codex。持久化后端缺失、失败、记录不完整或所有者取消启动检查点都会阻止启动，并在存储仍可用时发布失败或所有者取消终态。准入完成后，断开连接或刷新页面不会取消审查。接收 Agent 的上下文拥有后台进程；超时、Agent 卸载或插件卸载都会终止进程树，并等待整棵进程树退出后再记录终态。所有者卸载会启用独立的 `teardownTimeoutMs` 截止时间，同时约束 stdout 关闭、子进程完成、进程树退出确认与生命周期持久化，因此不结算的子进程观察器、flush listener 或后端读取会转成显式卸载失败。如果子进程 spawn 同步触发所有者卸载后再抛出异常，审查会按所有者取消结算，而不是记为启动失败。只有从持久化存储精确读回 `review/end` 后才会释放并发名额。精确读回后，其他 `session/flush` listener 的失败会单独记录，但不会继续占用已完成审查的名额。子进程结算或退出确认超时或失败、终态追加失败或终态精确读回失败都会让运行继续由所有者持有，并让卸载显式报告失败。无法在自身宿主退出时终止进程树的子进程提供方会在 Codex 启动前拒绝。随附的本地提供方只通过 Windows kill-on-close Job Object 准入该要求；其 POSIX guardian 无法约束 `setsid()` 后代，因此会在 Codex 启动前拒绝。后续宿主恢复会话时，恢复逻辑首先把既没有 `review/start`、也没有 `command/done` 的每条 `/review` `command/run` 结算为通用命令错误。fork 继承的前缀使用 `Review was not continued in this fork; the source session review is unaffected.`；其他前缀使用 `Review interrupted because its previous host stopped before recording admission.`，关闭宿主在可执行文件解析或启动记录发布期间退出留下的通用命令行，而不会虚构已准入审查。恢复逻辑随后会为每个缺少命令确认的 `review/start` 补充一条指向该启动事件的成功 `command/done`，包括已经存在终态的审查。只有非 fork 的开放审查才会继续追加一条 `interrupted` 终态；持久化保证表明其旧进程树已经失去执行能力。fork 继承开放审查时，子会话会改为记录该运行没有在 fork 中继续，源会话保持不变。压缩仍在同一会话与所有者中进行，不会迁移或结束审查。

Web 客户端把这些事件重建为一张独立审查卡片。回放时会按外部条目 id 索引活动，并在发布时按首次出现顺序生成快照。卡片会本地化类别，并显示真实 Codex 推理、命令、MCP 工具、网页搜索、文件修改和消息生成的安全条目摘要；它不会暴露隐藏推理文本，也不会虚构百分比。刷新后回放会得到相同的运行中或终态卡片；恢复或 fork 继承出的未闭合审查会显示为「已中断」，并携带已记录的原因。成功的 `command/done.sourceEventSeq` 指向 `review/start`，审查节点还会认领带品牌类型的命令身份，因此分页窗口尚未加载该启动事件时，通用命令行仍不会重复显示。
