...
| 在 POSIX 上使用随附本地提供方执行 `/review` | 准入后输入框会清空，随后持久卡片以 `The review could not start: subprocess-local: host-death termination requires Windows Job Object ownership` 失败；Codex 不会启动。 |
...
Web 客户端把这些事件重建为一张独立审查卡片。卡片展示真实分析阶段，以及命令、MCP 工具、网页搜索、文件修改和消息生成的安全摘要；它不会暴露隐藏推理文本，也不会虚构百分比。刷新后重放会得到相同的运行中或终态卡片；恢复或 fork 继承出的未闭合审查会显示为「已中断」，并携带已记录的原因。成功的 `command/done.sourceEventSeq` 指向 `review/start`，因此更丰富的领域记录出现后，通用命令行不会重复显示。
...
- **随附本地提供方的宿主死亡保证仅支持 Windows**：本地子进程提供方会在 POSIX 上于 Codex 启动前拒绝 `/review`；POSIX 部署需要由 cgroup、supervisor 或其他能够约束守护化后代的所有权机制支撑子进程提供方。
