...
每次成功准入的调用都会依次记录 `review/start`、零到多个从 Codex JSONL 解码出的 `review/activity`，以及一个 `review/end`；同时仍有执行器持有的纯日志事件对 `command/run` / `command/done`。该命令关闭通用输入记录，因此 `command/run` 省略 `args`；`review/start.focus` 是成功准入后关注点的唯一持久副本。`review/start.request` 保存本次 Codex 请求实际使用的完整提示词、规范 argv、显式子进程环境项、工作目录与运行超时。对话记录会排除指令、目录和运行时快照上下文，避免把这些配置误记为用户对话；直接消息、模型输出、工具活动、通知、转发、召回和压缩摘要仍作为审查证据。这些事件都不会进入被审查 Agent 的模型历史。处理器会在写入 `review/start` 前立即提交请求准入，因此浏览器断开不会留下“审查仍在运行、`command/done` 却为错误”的矛盾记录；该记录写入后，断开连接或刷新页面不会取消审查。接收 Agent 的上下文拥有后台进程；超时、Agent 卸载或插件卸载都会终止进程树，并等待整棵进程树退出后再记录终态。如果无法确认整棵进程树已经退出，本次运行不会发布误导性的终态，会继续占用并发名额，并在卸载期间报告清理失败。后续宿主恢复一个含有未闭合 `review/start` 的会话时，会先补充一条指向该启动事件的成功 `command/done`（若缺失），再追加一条 `interrupted` 终态；此前的进程已经不可能仍在运行，因此两个持久生命周期会同时闭合。
