...
每次成功准入的调用都会依次记录 `review/start`、零到多个从 Codex JSONL 解码出的 `review/activity`，以及一个 `review/end`；同时仍有执行器持有的纯日志事件对 `command/run` / `command/done`。每条终态记录都包含至少一个非空白字符的结果或诊断。该命令关闭通用输入记录，因此 `command/run` 省略 `args`；`review/start.focus` 是成功准入后关注点的唯一持久副本。`review/start.request` 保存本次 Codex 请求实际使用的完整提示词、规范 argv、显式子进程环境项、工作目录、必需的 `hostDeath: "terminate"` 行为与运行超时。对话记录会排除指令、目录和运行时快照上下文，避免把这些配置误记为用户对话；直接消息、模型输出、工具活动、通知、转发、召回和压缩摘要仍作为审查证据。这些事件都不会进入被审查 Agent 的模型历史。处理器会在写入 `review/start` 前立即提交请求准入，要求会话存储执行 flush，再从 `ctx.sessionPersistence` 的新事件序号开始读取；只有存储事件与该 `review/start` 完全匹配且所有 flush listener 都成功时才会启动 Codex。持久化后端缺失、失败或记录不完整都会阻止启动，并在存储恢复后发布失败终态。准入完成后，断开连接或刷新页面不会取消审查。接收 Agent 的上下文拥有后台进程；超时、Agent 卸载或插件卸载都会终止进程树，并等待整棵进程树退出后再记录终态。如果子进程 spawn 同步触发所有者卸载后再抛出异常，审查会按所有者取消结算，而不是记为启动失败。只有从持久化存储精确读回 `review/end` 后才会释放并发名额。精确读回后，其他 `session/flush` listener 的失败会单独记录，但不会继续占用已完成审查的名额。退出确认、终态追加或终态精确读回失败都会让运行继续由所有者持有，并让卸载显式报告失败。无法在自身宿主退出时终止进程树的子进程提供方会在 Codex 启动前拒绝。随附的本地提供方只通过 Windows kill-on-close Job Object 准入该要求；其 POSIX guardian 无法约束 `setsid()` 后代，因此会在 Codex 启动前拒绝。后续宿主恢复会话时，恢复逻辑会为每个缺少命令确认的 `review/start` 补充一条指向该启动事件的成功 `command/done`，包括已经存在终态的审查。只有非 fork 的开放审查才会继续追加一条 `interrupted` 终态；持久化保证表明其旧进程树已经失去执行能力。fork 继承开放审查时，子会话会改为记录该运行没有在 fork 中继续，源会话保持不变。压缩仍在同一会话与所有者中进行，不会迁移或结束审查。
