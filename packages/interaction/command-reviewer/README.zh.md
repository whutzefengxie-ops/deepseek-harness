...
| `teardownTimeoutMs` | `10000` | 从 Agent 或插件卸载开始，到进程树退出确认与生命周期持久化完成的最长毫秒数；受 Node 定时器上限约束。 |
