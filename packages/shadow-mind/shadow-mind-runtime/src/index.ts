...
export class ShadowMindRuntime extends Service {
  ...
  status(agent: Agent): ShadowMindStatus {
    ...
    const state = this.owners.get(agent)
    if (state === undefined) return { paused: false, active: [], pendingSchedules: 0, epoch: 0 }
    return {
      paused: state.paused,
  ...
  private launch(
    ...
    entry.done = this.runShadow(agent, state, entry, definition).finally(() => {
  ...
  private async runShadow(
    ...
    if (output === undefined || output.status !== 'report' || !this.accepts(agent, state, entry.epoch)) return
