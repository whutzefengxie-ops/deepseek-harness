...
`review/start` 到达后卡片立即出现。卡片展示可选关注点、当前状态，以及分析、命令、工具、搜索、文件修改与结果生成对应的真实 Codex JSONL 条目活动。没有安全条目详情的类别会按当前界面语言本地化。没有活动的运行中卡片会说明正在等待 Codex 事件；终态卡片不会保留这条等待文案。审查进入任一终态后，缺少自身完成事件的活动会显示为「已停止」。活动详情只包含安全的操作摘要，不显示隐藏推理文本，也不虚构百分比。`review/end` 会加入最终 Markdown 审查，或失败、取消、宿主中断诊断。

宿主命令返回启动记录的 `sourceEventSeq`，审查节点还以同一个带品牌类型的命令身份声明呈现所有权。通用斜杠命令 Definition 会按启动锚点或命令身份隐藏重复的成功行，包括分页历史尚未加载 `review/start` 时。
