...
`review/start` 到达后卡片立即出现。卡片展示可选关注点、当前状态，以及分析、命令、工具、搜索、文件修改与结果生成对应的真实 Codex JSONL 活动。活动详情只包含安全的操作摘要，不显示隐藏推理文本，也不虚构百分比。`review/end` 会加入最终 Markdown 审查或失败、取消诊断。
...
- 刷新可以重建持久化事件，但无法重新连接原始 stdout；后续宿主事件仍会继续更新同一节点。
