...
本包自带的卡片覆盖 shell 执行器（`bash`）、agent 循环的工具调用并行度（`agent-loop`）、DeepSeek 搜索提供方（`web-search-deepseek`）以及 Codex 审查者命令（`command-reviewer`）。
...
字段是否被覆盖，取决于它是否出现在原始用户层中，而非取决于它的值；重置会清除该字段，使其重新继承组装层。审查者的模型、提示词和上下文控件在清空后会存储显式空字符串，因为空值会禁用从组装层继承的值；这些控件的「重置」操作会删除覆盖。secret 角色的字段绝不搭乘响应，因此密钥控件初始为空、只报告是否已配置，并经由 credentials 领域而非 settings 分节写入；空草稿不写入任何东西，保留已存密钥。
