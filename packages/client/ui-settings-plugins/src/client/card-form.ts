 ...
 * draft clears the field unless `emptyWrite` preserves it as an explicit
 * empty-string override.
 * @param field - field name inside the namespace section.
 * @param pattern - optional complete-value pattern for non-empty drafts.
 * @param emptyWrite - whether an empty draft clears or explicitly empties the field.
 * @returns the field's conversion spec.
 */
export function textField(
  field: string,
  pattern?: RegExp,
  emptyWrite: 'clear' | 'set' = 'clear',
): CardFieldSpec {
  return {
    field,
    ...
    parse: (text) => {
      const trimmed = text.trim()
      if (trimmed === '') return emptyWrite === 'set' ? { kind: 'set', value: '' } : { kind: 'clear' }
