...
- The spec is fully explicit — argv, cwd, per-stream stdio dispositions, Host-death behavior, grace — because deployment-varying defaults belong to the caller's config, not to a hidden subprocess-service default (the `dsh-shell` request/spec split is the owning template). `hostDeath: 'terminate'` requires the provider to prevent the tree from retaining execution ability after the provider Host exits and to reject before the requested command starts when that guarantee is unavailable; `'allow'` accepts provider-specific survivors. `argv` is never shell-interpreted; a consumer that wants a shell passes `['bash', '-c', command]` itself.
