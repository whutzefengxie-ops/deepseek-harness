...
- **部分进程形式在 JavaScript 无法运行时仍需要外部所有权**：要求宿主死亡后无存活进程保证的 POSIX 命令、由调用方持有原始 stdin 管道的运行、terminal session 与打包单文件运行时，需要外部 supervisor、容器或等价 OS 所有者。进程内清理会通过 Node 同步 `exit` 事件覆盖直接 `process.exit()`、默认未捕获异常和默认未处理 rejection；POSIX guardian 也会在宿主意外丢失后尝试清理进程组，但无法约束已经离组的 daemon。
