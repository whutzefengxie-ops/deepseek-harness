...
flowchart LR
  ...
  svc_shadowMind["ctx.shadowMind<br/>Root background review orchestration"]
  ...
  pkg_shadow_mind_runtime --> svc_shadowMind
  pkg_shell --> svc_shell
  ...
  svc_shadowMind --> pkg_tool_shadow_mind
...
| `ctx.shadowMind` | `core` | [`shadow-mind-runtime`](../packages/shadow-mind/shadow-mind-runtime) | - | [`tool-shadow-mind`](../packages/shadow-mind/tool-shadow-mind) | - | 观察已完成且使用工具的 root 轮次，启动数量受限的全新一次性审查器，并中继已接受的持久化报告；管理插件消费其定义、设置和每个 root 的控制 API。 |
