...
describe('CI workflow', () => {
  ...
  it('keeps required Wine and reviewer jobs, a non-blocking native job with failover, and a master-only standby', () => {
    const workflow = loadWorkflow('.github/workflows/ci.yml')
    const masterWorkflow = loadWorkflow('.github/workflows/ci-master.yml')
    if (!isRecord(workflow.jobs)
      || !isRecord(workflow.jobs.windows)
      || !isRecord(workflow.jobs['windows-reviewer'])
      ...
      throw new TypeError('CI workflow must define windows, windows-reviewer, windows-native, node-24, node-24-coverage, node-24-consumers, and all-checks-passed; ci-master must define wine-apt-cache and serial-windows')
    }

    const windows = workflow.jobs.windows
    const windowsReviewer = workflow.jobs['windows-reviewer']
    ...
    if (!Array.isArray(windows.steps) || !Array.isArray(windowsReviewer.steps) || !Array.isArray(aggregate.needs)) {
    ...
    // Required native-kernel reviewer lane: built product, real Chromium, and
    // both visible lifecycle and force-killed Host recovery paths.
    expect(windowsReviewer['runs-on']).toBe('windows-2025')
    expect(windowsReviewer.name).toBe('windows node 24 / reviewer blocking')
    expect(windowsReviewer.if).toBe("github.event_name == 'pull_request'")
    const reviewerCommands = windowsReviewer.steps
      .filter((step): step is Record<string, unknown> & { run: string } => (
        isRecord(step) && typeof step.run === 'string'
      ))
      .map(step => step.run)
    expect(reviewerCommands).toContain('pnpm run build')
    expect(reviewerCommands.some(command => command.includes('playwright install chromium'))).toBe(true)
    expect(reviewerCommands.some(command => (
      command.includes('apps/web/tests/reviewer-lifecycle.e2e.ts')
      && command.includes('apps/web/tests/reviewer-host-recovery.e2e.ts')
    ))).toBe(true)
    ...
    // Aggregate: Wine and focused reviewer jobs required, complete native job excluded.
    expect(aggregate.needs).toContain('windows')
    expect(aggregate.needs).toContain('windows-reviewer')
