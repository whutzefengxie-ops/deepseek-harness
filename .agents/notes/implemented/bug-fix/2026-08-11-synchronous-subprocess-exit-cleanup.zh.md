...
| 无法执行 JavaScript 的宿主故障 | [父进程死亡 guardian](2026-08-20-parent-death-guardian-for-batch-subprocesses.zh.md)会尝试清理 POSIX 进程组；Windows kill-on-close Job 会持有全部后代 | Windows Job 退出证明约束完成；POSIX 无存活进程要求、原始 stdin pipe、terminal 与打包运行时需要 supervisor、容器或 OS 所有权 |
...
**把所有故障模式交给外部 supervisor。** 不接受将其作为唯一方案，因为 Node为几条常见致命路径提供可靠的同步回调，而 provider已经拥有精确目标。JavaScript 无法运行时，[父进程死亡 guardian](2026-08-20-parent-death-guardian-for-batch-subprocesses.zh.md)提供 POSIX 尽力进程组清理与 Windows Job 强约束；POSIX 无存活进程要求和被排除的进程形式仍需外部所有权。
...
listener 本身无法覆盖不执行 JavaScript 的故障。父进程死亡 guardian 会尝试清理 stdin 为 ignore 或批量输入的普通命令，而只有 Windows Job 所有权会准入无存活进程保证。POSIX 恢复敏感命令、原始 stdin pipe、terminal session、打包运行时，以及 provider 首次观察前已经逃逸的 terminal 后代仍保留文档规定的外部所有权要求。terminal 逃逸缺口仍由 Issue #1726 跟踪。
