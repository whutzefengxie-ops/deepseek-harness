...
| 无法执行 JavaScript 的宿主故障 | [父进程死亡 guardian](2026-08-20-parent-death-guardian-for-batch-subprocesses.md)会尝试清理 POSIX 进程组；Windows kill-on-close Job 会持有全部后代 | Windows Job 退出证明约束完成；POSIX 无存活进程要求、原始 stdin pipe、terminal 与打包运行时需要 supervisor、容器或 OS 所有权 |
...
**把所有故障模式交给外部 supervisor。** 不接受将其作为唯一方案，因为 Node为几条常见致命路径提供可靠的同步回调，而 provider已经拥有精确目标。JavaScript 无法运行时，[父进程死亡 guardian](2026-08-20-parent-death-guardian-for-batch-subprocesses.md)提供 POSIX 尽力进程组清理与 Windows Job 强约束；POSIX 无存活进程要求和被排除的进程形式仍需外部所有权。
