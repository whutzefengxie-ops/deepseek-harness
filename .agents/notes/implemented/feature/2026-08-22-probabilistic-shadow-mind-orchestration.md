...
Shadow Mind is an optional plugin family installed through the profile bundle `@deepseek-ai/dsh-shadow-mind`. The bundle mounts a root-only orchestration service, a separate management-tool plugin, and Web administration under **Settings → Plugins → Shadow Mind**. It observes existing Session and Agent extension points; `dsh-agent-loop` remains unchanged.
...
`packages/shadow-mind/shadow-mind-runtime` provides `@deepseek-ai/dsh-shadow-mind-runtime`, `ctx.shadowMind`, definition persistence, settings, scheduling, projection, child ownership, batching, `/shadow` state methods, the trusted administration Remote, and the durable report invariant. `packages/shadow-mind/tool-shadow-mind` provides `@deepseek-ai/dsh-tool-shadow-mind`, registers `/shadow status|pause|resume|toggle`, and contributes eight model tools for listing, creating, updating, enabling, disabling, deleting, reading settings, and updating settings. Settings updates atomically set or remove the supplied user overrides, including related budget fields. Mutations require the exact `allowed-once` approval outcome. Keeping these packages separate permits runtime-only deployments without model editing authority.

`packages/client/ui-shadow-mind` provides `@deepseek-ai/dsh-client-ui-shadow-mind`. It mounts the generated Remote contribution, edits the live settings namespace and Markdown definitions, controls the selected root, displays process-local recent-run evidence after active work settles, presents accepted reports and their root follow-up in conversation, and turns `/shadow` results into composer notices that remain visible for blank Sessions. `packages/bundle/shadow-mind` provides the installable `@deepseek-ai/dsh-shadow-mind` patch layer. `dsh plugin --profile <profile> add @deepseek-ai/dsh-shadow-mind` records the dependency and bundle in the selected profile; the patch mounts runtime, management tools, and Web administration in that order.

## Differences from Pi

DSH intentionally uses its existing `read`, `grep`, and `glob` tools instead of Pi's `read`, `grep`, `find`, and `ls`. Arguments are redacted by default, and no known or unknown tool result includes a first-line preview. Structured terminal output replaces the `NOT_RELEVANT` sentinel and `report_to_main`. Model eligibility accepts local `*` and `?` globs against model ids or full routes, which is broader than Pi's exact full-route rule. Child Sessions follow DSH persistence policy instead of switching to memory when debug is false. DSH provides settings and definition administration through its Plugins settings section, renders durable report batches with child provenance in conversation, and marks the root reply that consumed them. It retains generic approval and command presentation and does not add Pi's `Alt+S` shortcut.
...
The management package coverage proves Loader-compatible named exports, eight tool and command registrations, HMR disposal, refusal without mutation, and `allowed-once` create, update, enable, disable, delete, and settings writes. Browser coverage pins Remote mount disposal, settings and definition administration, selected-root controls, blank-Session `/shadow` notices, durable report cards, child provenance, and root-reply markers. The bundle composition test parses the shipped `cordis.patch.yml` through the real Loader and activates all three entries. A keyless assembled snapshot pins the complete root-tool, child-report, durable-relay, follow-up, and headless-convergence transcript.
