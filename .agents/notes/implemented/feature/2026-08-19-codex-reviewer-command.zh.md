...
组合后的 Web 生命周期测试会在原生 Windows 上通过 Chromium 提交命令并检查持久化会话日志，证明受 Job 持有的 Codex 启动、公开活动、完成与刷新重放。在 POSIX 上，测试改为证明默认命令目录和设置命名空间不包含不受支持的审查者，且刷新前后都不会出现 Codex 进程或审查卡片。[Windows CI 拓扑](../process/2026-08-08-native-windows-pull-request-ci.md)负责这些平台结果的运行位置。
