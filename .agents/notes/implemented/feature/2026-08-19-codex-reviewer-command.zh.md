...
活动身份使用互不重叠的插件与外部命名空间：合成 turn 使用 `turn:main`，每个 Codex 条目存为 `item:<外部 id>`，因此外部条目无法与 turn 生命周期碰撞。可见审查节点会立即通过 `CommandId` 认领通用命令行，无须等待 `command/done`；成功命令的来源锚点仍作为分页重放的第二条认领路径。
