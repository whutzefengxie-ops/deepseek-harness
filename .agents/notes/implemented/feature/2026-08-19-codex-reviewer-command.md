...
The assembled Web lifecycle test submits the command through Chromium and inspects the persisted Session log on both supported host outcomes. Native Windows proves Job-owned Codex start, public activity, completion, and refresh replay; POSIX proves the local provider starts no Codex process and replays the durable failure. The [Windows CI topology](../process/2026-08-08-native-windows-pull-request-ci.md) owns where these platform results run.
